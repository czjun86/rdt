 ...
 * @版本：V1.0
 */
@Service("comprehensiveESDao")
public class ComprehensiveESDao {

	private static final Logger logger = LoggerFactory.getLogger(ComprehensiveESDao.class);
	// 分组维度(0否1是)
	...
	public List<Map<String, Object>> queryComprehensiveByES(Map<String, Object> params, List<RoadInfo4ESBean> listRoadInfos, List<KpiSettings> kpiInfos) throws Exception {
		...
		System.out.println("解析结果，不包含分页消耗的时间:"+(edLong-stLong));
	...
	private List<Map<String, Object>> fetchResponseResult(String jsonResult) {
		...
		for(int i = 0;i<timeBuckets.length();i++){
			...
			for(int k=0;k<areaBuckets.length();k++){
				...
				for(int j=0;j<roadBuckets.length();j++){
					...
					for(int m=0;m<tmsBuckets.length();m++){
						...
						// lost_avg
						String lostAvg = tmsJSONObject.getJSONObject("lost_avg").get("value").toString();
						mapResult.put("ping_lose_rate", NumUtils.isNumber(lostAvg) ? NumUtils.resDouble(NumberUtils.toDouble(lostAvg), 2) : "N/A");
						// rsrp_avg
						String rsrpAvg = tmsJSONObject.getJSONObject("rsrp_avg").get("value").toString();
						mapResult.put("lte_rsrp", NumUtils.resDouble(NumberUtils.toDouble(rsrpAvg), 2));
						// http_avg
						String httpAvg = tmsJSONObject.getJSONObject("http_avg").get("value").toString();
						...
						// delay_avg
						String delayAvg = tmsJSONObject.getJSONObject("delay_avg").get("value").toString();
						mapResult.put("ping_avg_delay", NumUtils.isNumber(delayAvg) ? df0.format(NumberUtils.toDouble(delayAvg))  : "N/A");
						
						// time
						if (timeAgg == 1) {
							// endTime
							double et = tmsJSONObject.getJSONObject("endTime").getDouble("value");
							//转化科学计数法为Long型
						...
						//网络掉线率
						String networkRate = tmsJSONObject.getJSONObject("netdrops_lv").get("value").toString();
						...
						//业务掉线率
						String pingRate = tmsJSONObject.getJSONObject("busdrops_lv").get("value").toString();
						...
						if(rsrpItl == 1){
							for(int f=1;f<=6;f++){
								String val = tmsJSONObject.getJSONObject("rsrp_interval_"+f).get("value").toString();
								mapResult.put("rsrp_ratio"+f, NumUtils.resDouble(NumberUtils.toDouble(val)*100, 2));
								String con = tmsJSONObject.getJSONObject("rsrp_count_"+f).get("value").toString();
						...
						if(snrItl == 1){
							for(int f=1;f<=6;f++){
								String val = tmsJSONObject.getJSONObject("snr_interval_"+f).get("value").toString();
								mapResult.put("snr_ratio"+f, NumUtils.resDouble(NumberUtils.toDouble(val)*100, 2));
								String con = tmsJSONObject.getJSONObject("snr_count_"+f).get("value").toString();
						...
						if(rsrqItl == 1){
							for(int f=1;f<=6;f++){
								String val = tmsJSONObject.getJSONObject("rsrq_interval_"+f).get("value").toString();
								mapResult.put("rsrq_ratio"+f, NumUtils.resDouble(NumberUtils.toDouble(val)*100, 2));
								String con = tmsJSONObject.getJSONObject("rsrq_count_"+f).get("value").toString();
						...
						if(httpItl == 1){
							for(int f=1;f<=6;f++){
								String val = tmsJSONObject.getJSONObject("http_interval_"+f).get("value").toString();
								mapResult.put("bw_link_ratio"+f, NumUtils.resDouble(NumberUtils.toDouble(val)*100, 2));
								String con = tmsJSONObject.getJSONObject("http_count_"+f).get("value").toString();
						...
						if(delayItl == 1){
							for(int f=1;f<=6;f++){
								String val = tmsJSONObject.getJSONObject("delay_interval_"+f).get("value").toString();
								mapResult.put("ping_delay_ratio"+f, NumUtils.resDouble(NumberUtils.toDouble(val)*100, 2));
								String con = tmsJSONObject.getJSONObject("delay_count_"+f).get("value").toString();
						...
						if(lostItl == 1){
							for(int f=1;f<=6;f++){
								String val = tmsJSONObject.getJSONObject("lost_interval_"+f).get("value").toString();
								mapResult.put("ping_lose_ratio"+f, NumUtils.resDouble(NumberUtils.toDouble(val)*100, 2));
								String con = tmsJSONObject.getJSONObject("lost_count_"+f).get("value").toString();
	...
	private String qryCondition(QueryBean bean,List<KpiSettings> kpiInfos) throws Exception {
        ...
        filters.put(new JSONObject("{'range':{'test_time': {'from': '"+stime+"','to': '"+etime+"','include_lower': true,'include_upper': true}}}"));
        //排除没有归属到道路点的采样点
        filters.put(new JSONObject("{'not': {'terms': {'point_code': ['']}}}"));
		// 运营商
		if (null != operator) {
			if (operator.length() == 1 && !operator.equals("-1")) { // 单个运营商
				filters.put(new JSONObject("{'terms': {'telecoms': ['"+operator+"']}}"));
				...
				if (!authOpt.equals("")) {
					...
					switch (opt.length) {
					case 1:
						filters.put(new JSONObject("{'terms': {'telecoms': ['"+opt[0]+"']}}"));
						break;
					case 2:
						filters.put(new JSONObject("{'terms': {'telecoms': ['"+opt[0]+"','"+opt[1]+"']}}"));
						break;
					case 3:
						filters.put(new JSONObject("{'terms': {'telecoms': ['"+opt[0]+"','"+opt[1]+"','"+opt[2]+"']}}"));
						break;
					default:
						break;
					}
			...
			if (null != area && !area.equals("-1")) {
				filters.put(new JSONObject("{'prefix': {'region_code': '"+area+"'}}"));
			} else if (null != district && !district.equals("-1")) {
				filters.put(new JSONObject("{'prefix': {'region_code': '"+district.substring(0, 4)+"'}}"));
			} else if (null != province && !province.equals("-1")) {
				filters.put(new JSONObject("{'prefix': {'region_code': '"+province.substring(0, 2)+"'}}"));
			}
		}
		// 道路等级
		if (null != roadLevel && roadLevel != -1) {
			filters.put(new JSONObject("{'term': {'road_level': "+roadLevel+"}}"));
		}
		// 道路类型
		if (null != roadType && roadType != -1) {
			filters.put(new JSONObject("{'term': {'road_type': "+roadType+"}}"));
		}
		// 道路ID
		if (null != roadId && !roadId.equals("-1")) {
			filters.put(new JSONObject("{'term': {'road_id': '"+roadId+"'}}"));
		}
		...
		if(snrItl == 1){
			...
			for(int i=0;i<list.size();i++){
				...
				temp.append("\"snr_itl_"+(i+1)+"\":{\"bucket_script\":{\"buckets_path\":{\"itl_val\":\"snr_avg2\"},");
				temp.append("\"script\":\"if(itl_val "+opt1+" "+stv+" && itl_val "+opt2+" "+edv+"){return 1;}else{return 0;}\"}},");
			}
		}
		...
		if(rsrqItl == 1){
			...
			for(int i=0;i<list.size();i++){
				...
				temp.append("\"rsrq_itl_"+(i+1)+"\":{\"bucket_script\":{\"buckets_path\":{\"itl_val\":\"aggRsrqFilter>rsrq_avg2\"},");
				temp.append("\"script\":\"if(itl_val "+opt1+" "+stv+" && itl_val "+opt2+" "+edv+"){return 1;}else{return 0;}\"}},");
			}
		}
		...
		if(httpItl == 1){
			...
			for(int i=0;i<list.size();i++){
				...
				temp.append("\"http_itl_"+(i+1)+"\":{\"bucket_script\":{\"buckets_path\":{\"itl_val\":\"aggHttpFilter>http_avg2\"},");
				temp.append("\"script\":\"if(itl_val "+opt1+" "+stv+" && itl_val "+opt2+" "+edv+"){return 1;}else{return 0;}\"}},");
			}
		}
		...
		if(delayItl == 1){
			...
			for(int i=0;i<list.size();i++){
				...
				temp.append("\"delay_itl_"+(i+1)+"\":{\"bucket_script\":{\"buckets_path\":{\"itl_val\":\"delayFilter>delay_avg2\"},");
				temp.append("\"script\":\"if(itl_val "+opt1+" "+stv+" && itl_val "+opt2+" "+edv+"){return 1;}else{return 0;}\"}},");
			}
		}
		...
		if(lostItl == 1){
			...
			for(int i=0;i<list.size();i++){
				...
				temp.append("\"lost_itl_"+(i+1)+"\":{\"bucket_script\":{\"buckets_path\":{\"itl_val\":\"loseFilter>lost_avg2\"},");
				temp.append("\"script\":\"if(itl_val "+opt1+" "+stv+" && itl_val "+opt2+" "+edv+"){return 1;}else{return 0;}\"}},");
			}
		}
		...
		if(rsrpItl == 1){
			temp.append("\"rsrp_un_count\":{\"bucket_script\":{\"buckets_path\":{\"mc1\": \"rsrp_count_1\",\"mc2\":");
			temp.append("\"rsrp_count_2\",\"mc3\": \"rsrp_count_3\",\"mc4\": \"rsrp_count_4\",\"mc5\": \"rsrp_count_5\",");
			temp.append("\"mc6\": \"rsrp_count_6\"},\"script\":\"mc1 + mc2 + mc3 + mc4 + mc5 + mc6\"}},");
			for(int i=0;i<6;i++){
				...
				temp.append("\"unmc\":\"rsrp_un_count\"},\"script\":\"mc / unmc\"}},");
			}
		}
		...
		if(snrItl == 1){
			temp.append("\"snr_un_count\":{\"bucket_script\":{\"buckets_path\":{\"mc1\": \"snr_count_1\",\"mc2\":");
			temp.append("\"snr_count_2\",\"mc3\": \"snr_count_3\",\"mc4\": \"snr_count_4\",\"mc5\": \"snr_count_5\",");
			temp.append("\"mc6\": \"snr_count_6\"},\"script\":\"mc1 + mc2 + mc3 + mc4 + mc5 + mc6\"}},");
			for(int i=0;i<6;i++){
				...
				temp.append("\"unmc\":\"snr_un_count\"},\"script\":\"mc / unmc\"}},");
			}
		}
		...
		if(rsrqItl == 1){
			temp.append("\"rsrq_un_count\":{\"bucket_script\":{\"buckets_path\":{\"mc1\": \"rsrq_count_1\",\"mc2\":");
			temp.append("\"rsrq_count_2\",\"mc3\": \"rsrq_count_3\",\"mc4\": \"rsrq_count_4\",\"mc5\": \"rsrq_count_5\",");
			temp.append("\"mc6\": \"rsrq_count_6\"},\"script\":\"mc1 + mc2 + mc3 + mc4 + mc5 + mc6\"}},");
			for(int i=0;i<6;i++){
				...
				temp.append("\"unmc\":\"rsrq_un_count\"},\"script\":\"mc / unmc\"}},");
			}
		}
		...
		if(httpItl == 1){
			temp.append("\"http_un_count\":{\"bucket_script\":{\"buckets_path\":{\"mc1\": \"http_count_1\",\"mc2\":");
			temp.append("\"http_count_2\",\"mc3\": \"http_count_3\",\"mc4\": \"http_count_4\",\"mc5\": \"http_count_5\",");
			temp.append("\"mc6\": \"http_count_6\"},\"script\":\"mc1 + mc2 + mc3 + mc4 + mc5 + mc6\"}},");
			for(int i=0;i<6;i++){
				...
				temp.append("\"unmc\":\"http_un_count\"},\"script\":\"mc / unmc\"}},");
			}
		}
		...
		if(delayItl == 1){
			temp.append("\"delay_un_count\":{\"bucket_script\":{\"buckets_path\":{\"mc1\": \"delay_count_1\",\"mc2\":");
			temp.append("\"delay_count_2\",\"mc3\": \"delay_count_3\",\"mc4\": \"delay_count_4\",\"mc5\": \"delay_count_5\",");
			temp.append("\"mc6\": \"delay_count_6\"},\"script\":\"mc1 + mc2 + mc3 + mc4 + mc5 + mc6\"}},");
			for(int i=0;i<6;i++){
				...
				temp.append("\"unmc\":\"delay_un_count\"},\"script\":\"mc / unmc\"}},");
			}
		}
		...
		if(lostItl == 1){
			temp.append("\"lost_un_count\":{\"bucket_script\":{\"buckets_path\":{\"mc1\": \"lost_count_1\",\"mc2\":");
			temp.append("\"lost_count_2\",\"mc3\": \"lost_count_3\",\"mc4\": \"lost_count_4\",\"mc5\": \"lost_count_5\",");
			temp.append("\"mc6\": \"lost_count_6\"},\"script\":\"mc1 + mc2 + mc3 + mc4 + mc5 + mc6\"}},");
			for(int i=0;i<6;i++){
				...
				temp.append("\"unmc\":\"lost_un_count\"},\"script\":\"mc / unmc\"}},");
			}
		}
		...
		aggFilter.put("aggs",new JSONObject(temp.toString()));
       ...
       //删除多余的桶内容
        ...
        long edLong = new Date().getTime();
		System.out.println("replace中间结果消耗时间:"+(edLong-stLong));
