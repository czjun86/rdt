...
<body class="page-header-fixed">
					...
					<!-- title-->
						<div class="portlet ">
							<div class="portlet-body">
								...
								<div class="navbar navbar-default" role="navigation" method="post" action="${application.getContextPath()}/collection/collection" style="background:#fff !important;">
									<form class="navbar-form form-inline navbar-left breadcrumb"  id="collectionForm" >
										...
										<div class="form-group" style="margin-left:8px;">
											...
											<input class="form-control" type="text" id="collectionName" name="collectionName" style="width:160px !important;" value="${(collectionName)!}" placeholder="请输入方案名称"/>
										</div>
										<!-- 采集方案  结束 -->
										<!-- 维度选择 结束 -->
								...
								<!-- 查询条件 -->
								<form  action="${application.getContextPath()}/collection/collection" id="pageForm" role="search" method="post" >
									<!--时间-->
									<input type="hidden" id="queryTime_query" name="queryTime" value="${(queryTime)!}"/>
									<!--方案名称-->
									<input type="hidden" id="queryName" name="collectionName" value="${(collectionName)!}"/>
											...
											<#if page?? && page.list?? &&  page.list?size &gt; 0>
												<#list page.list as ls>
													<tr style="height:37px;<#if ls_index==page.list?size-1>border-bottom:1px #dddddd  solid;</#if>">
														<td style="text-align:center;" >${(ls.name)!'-'}</td>
														...
														<a href="#" class="editCollection" nid = "${(ls.id)!}" nname = "${(ls.name)!'-'}">编辑</a>
														&nbsp;|&nbsp;
														<a href="#" class="deleteCollection" nid = "${(ls.id)!}" nname = "${(ls.name)!'-'}">删除</a>
														</td>
													</tr>
												</#list>
											<#else>
												<tr>
													<td colspan="4"  align='center' style="height:37px;border-bottom:1px #dddddd  solid;">无采集方案</td>
												</tr>
											</#if>
										</tbody>
										<!-- 表单内容 结束 -->
	...
	<!--dialog请求层-->
	<!--<div id="ajax-modal" data-backdrop="static" data-keyboard="false" class="modal fade" tabindex="-1"></div>-->
	<div id="ajax-modal" class="modal fade" tabindex="-1"></div>
	<div id="ajax" class="modal fade" tabindex="-1" role="basic" aria-hidden="true"></div>
	<!-- 开始    日期范围控件脚本  -->
	...
	<script src="${application.getContextPath()}/js/collection/collection.js" type="text/javascript"></script>
	<script src="${application.getContextPath()}/js/collection/validater.js" type="text/javascript"></script>
