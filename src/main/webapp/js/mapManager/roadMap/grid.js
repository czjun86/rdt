...
// 		$("#h_1").hide();
	...
	//叠加轨迹静音联动
	$("#chooseType").die().live("change", function(e) {
		var chooseType=$(this).val();
		if(chooseType==1){
			$(".choseTime").show();
			$(".dubiOperator").hide();
			DateRanges.init('reportrange2');//加载时间控件
		}
      if(chooseType==2){
    	  $(".choseTime").hide();
		  $(".dubiOperator").show();
...
function getVoBean(){
	...
	bean.warn=$("#warn").val();
	bean.warnSymbol=$("#warnSymbol").val();
	var warnValue = $("#warnValue").val();
	bean.warnValue=$("#warnValue").val();
