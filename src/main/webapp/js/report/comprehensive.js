$(function(){
	...
	$("#taskbtn").click(function(){
		if(getCondition()&&validataRoadName()){
			$(function () {
		        $.messager.prompt("任务名", "请为查询任务命名", function (data) {
		            if (data) {
		            	$("#modal-backdrop").show();
						$.ajax({
							type:"post",
							url:contextPath + "/taskReport/submitTask",
							data:{
								taskName:data,
								threshold:$("#threshold_page").val(),
								...
								//获取运营商
								operator:$("#operator_page").val(),
								operatorName:$("#operator").find("option:selected").text(),
							...
							success:function(data){
								$("#modal-backdrop").hide();
								if(data =="scc"){
									$.messager.alert('提示',"任务提交成功,Spark努力计算中...","info");
								}else{
									$.messager.alert('提示',"任务提交失败!","error");
								}
							},
							error:function(data){
								$("#modal-backdrop").hide();
		            	...
		            	if(data==''){
		            		$.messager.alert('提示',"任务提交失败,任务名不能为空!","error");
		            	}
		            }
		        });
		    });
