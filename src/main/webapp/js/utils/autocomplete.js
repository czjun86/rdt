...
$('.appName').bind("propertychange input focus",function(event){
	$('.appName').addClass('spinner');//增加等待查询图标
    $this=$(this);
    if(event.type!='focus'){
    ...
    if($this.val()!=null && $this.val().replace(/^\s+|\s+$/g, '')!=""){
    	$.ajax({
    		type:"post",
    		url:contextPath + "/appTrend/getAppName",
    		data:({name:$this.val()}),//区分天，月
    		success:function(data){
    			$('.appName').removeClass('spinner');
    			var apps = data.apps;
    	        $tips.empty();
    	        ...
    	        for(var i=0;i<apps.length;i++){
    	            ...
    	            htmlcode+='<tr data-value="'+apps[i]+'"><td>'+apps[i]+'</td></tr>';
    	        }
...
$(document).delegate('.tips td','click',function(){
    ...
    if(tr.length==1){
        ...
        $inputfield=tr.parent().parent().parent().siblings('input.appName');
