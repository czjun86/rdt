...
*/
!function ($) {
    var DateRangePicker = function (idName,element, options, cb) {
        ...
        var DRPTemplate = '<div class="daterangepicker dropdown-menu" id="dtck_'+idName+'">' +
                ...
                '<div class="ranges">' +
                  '<div class="range_inputs">' +
                    ...
                    '<button class="' + this.buttonClasses + ' cancelBtn">' + this.locale.cancelLabel + '</button>' +
                  '</div>' +
                '</div>' +
              '</div>';
        ...
        if (hasOptions) {
            ...
            //根据文本框的值修改显示日历的初始化日期
            var tim=$('#'+idName+' input').val();
            if(tim!=null&&tim!=""){
            	var tim_st=tim.split("-")[0].trim();
            	var tim_et=tim.split("-")[1].trim();
            	this.startDate = moment(tim_st, this.format);
            	this.endDate = moment(tim_et, this.format);
            }    	
            ...
            //添加的控制日期格子显示
            if (typeof options.showDaysNumbers == 'boolean') {
                this.showDaysNumbers = options.showDaysNumbers;
            }
            
            //添加分钟数显示
            if (typeof options.showMinute == 'boolean'){
...
//            }
            //带上时间格式的判断
            if ((this.startDate.format(this.format)==this.endDate.format(this.format)) || (this.startDate.format(this.format)<this.endDate.format(this.format))) {
            	this.container.find('button.applyBtn').removeAttr('disabled');
            } else {
            	this.container.find('button.applyBtn').attr('disabled', 'disabled');
            }
        ...
        show: function (e) {
            ...
            $("#gis-tk-info").on('mousedown', $.proxy(this.hide, this));
            this.element.trigger('shown', {target: e.target, picker: this});
        },

        hide: function (e) {
            this.container.hide();
            //此处如果为失去焦点隐藏时需要修改时间值，就需要注释
            /**修改当前时间值分支 开始 (有一处修改点)**/
...
//            this.oldEndDate = this.endDate.clone();
            ...
            //此处如果为失去焦点隐藏时要修改时间值，就需要注释
            /**不修改当前时间值分支 开始 (有两处修改点)**/
            if (e) { //如果是失去焦点导致控件被隐藏,e会存在值
            ...
            $("#gis-tk-info").off('mousedown', this.hide);
        ...
        clickDate: function (e) {
            ...
            if (startDate.isSame(endDate) || startDate.isBefore(endDate)) {
                ...
                $(e.target).addClass('active');
                this.startDate = startDate;
                //如果开始时间小于结束时间，结束时间默认为开始时间加一天
                this.endDate = moment(startDate).add('day', 1).startOf('day');
              //如果开始时间小于结束时间，结束时间默认为开始时间同一天
        ...
        updateMonthYear: function (e) {
            ...
            //判断是否是不显示日期格子(纯粹的年月选择器)
            if(!this.showDaysNumbers){
            	var minM =  this.minDate.month();
            	var maxM =  this.maxDate.month();
            	var minY =  this.minDate.year();
                var maxY =  this.maxDate.year();
                
                //界限判定
                if((year+"."+(month<10?"0"+month:month))>(maxY+"."+(maxM<10?"0"+maxM:maxM))){//如果大于最日期则用最大日期
                	year = maxY;
                	month = maxM;
            	}else if((year+"."+(month<10?"0"+month:month))<(minY+"."+(minM<10?"0"+minM:minM))){//如果小于最小日期则用最小日期
            		year = minY;
            		month = minM;
            	}
            }
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate) {
            ...
            if (this.timePicker) {
                ...
                html += '</select>';
	              ...
	              //判断是否隐藏了分钟数下拉列表  
	              var fal='style = "display:none;"';
	              if(this.showMinute){
	            	  html += '<span> : </span>';
	            	  fal='';
	              }

                  html += '<select class="minuteselect" '+fal+' >';
    ...
    $.fn.daterangepicker = function (idName,options, cb) {
        this.each(function () {
            var el = $(this);
            if (el.data('daterangepicker_'+idName)){
            	//如果存在先删除原来的时间控件层
//            	$(".daterangepicker").remove();//原有的方法(为了不同时间粒度切换控件功能)
            	$("#dtck_"+idName).remove();//改造后方法(在为了不同时间粒度切换控件功能上，为了同一页面多个时间控件的功能)
            }
        	//新建时间控件层(为了不同时间粒度切换控件功能)
            el.data('daterangepicker_'+idName, new DateRangePicker(idName,el, options, cb));
